package com.myblog.springbootblogdemo.controller.filter;
...
public class JsonToUrlEncodedAuthenticationFilter implements Filter {
    ...
    public void doFilter(final ServletRequest request, final ServletResponse response, final FilterChain chain) {
        ...
        final String contentType = realRequest.getContentType().toLowerCase().chars()
                .mapToObj(c -> String.valueOf((char) c)).filter(x -> !x.equals(" ")).collect(Collectors.joining());
        ...
        if ((contentType.equals(MediaType.APPLICATION_JSON_VALUE.toLowerCase()))
            ...
            try (final BufferedReader br = new BufferedReader(new InputStreamReader(is), 16384)) {
                final String json = br.lines().collect(Collectors.joining(System.lineSeparator()));
